          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                {activeTab === 'supplier' && <th className="px-8 py-5 text-xs font-black text-slate-500 uppercase tracking-widest">Notes</th>}
                {activeTab !== 'companyAdvance' && (
            ...
            <tbody className="divide-y divide-slate-100">
              {paginatedData.length === 0 ? (
                <tr>
                  <td colSpan={(activeTab === 'loan' || activeTab === 'companyAdvance') ? (activeTab === 'companyAdvance' ? 4 : 5) : (activeTab === 'supplier' ? 6 : 5)} className="px-8 py-24 text-center">
                ...
                paginatedData.map((p: any) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      {activeTab === 'supplier' && (
                        <td className={`px-8 py-5 text-sm font-medium ${rowColor}`}>
                          {p.notes || '—'}
                        </td>
                      )}
                      {activeTab !== 'companyAdvance' && (
